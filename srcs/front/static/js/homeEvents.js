function extractOpponentFromResponse(response) {
  let opponent;
  if (response.game.playerLeft === Router.getUsername()) {
    opponent = response.game.playerRight;
  } else {
    opponent = response.game.playerLeft;
  }
  return opponent;
}

function extractOpponentIdFromResponse(response) {
  let opponent_id;
  if (response.game.playerLeft === Router.getUsername()) {
    opponent_id = response.game.playerRightId;
  } else {
    opponent_id = response.game.playerLeftId;
  }
  return opponent_id;
}
...
async function find1v1Game() {
  ...
  if (pause_games.status === 200 && pause_games_detail.length > 0) {
    ...
    if (game["detail"].length > 0) {
      let opponent = extractOpponentFromResponse(game);
      let opponent_id = extractOpponentIdFromResponse(game);
      ...
      alert.remove();
      Router.changePage("/pong?opponent=" + opponent_id);
    }
